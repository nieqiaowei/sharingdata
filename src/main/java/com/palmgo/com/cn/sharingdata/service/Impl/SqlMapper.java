...
public class SqlMapper {
     ...
     * @return
     */
    public String getNewSql(String sql,String remotejdbc_url) {
        String newSQL ;
        if(isState(remotejdbc_url)){
            newSQL = "<script>"+sql+"</script>";
        }else{
            newSQL = sql;
        }
    	return newSQL;
    ...
    public boolean isState(String remotejdbc_url){
        if(remotejdbc_url.startsWith("jdbc:avatica")){
            return false;
        }else{
            return true;
        }
